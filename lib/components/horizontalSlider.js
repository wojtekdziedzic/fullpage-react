...
const HorizontalSlider = ({ window, document, style, children, slides, isActive, isNext }) => {
  if (!isActive && !isNext) {
    return null;
  }

  const horizontalSliderStyle = Object.assign({}, (style || {}),
  {height: window.innerHeight, width: innerWidth, position: 'relative', overflowX: 'hidden', whiteSpace: 'nowrap', padding: '0px', margin: '0'});

  return (
    <div className="HorizontalSlider" style={horizontalSliderStyle}>
      {children}
      {slides.map((sl, i) => {
        const key = `horizontal-slide-${i}`;
        const style = { overflow: 'hidden', whiteSpace: 'normal', display: 'inline-block', height: window.innerHeight, width: window.innerWidth };

        const slProps = Object.assign({ key, style }, sl.props || {});
