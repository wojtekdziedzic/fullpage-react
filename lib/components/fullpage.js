...
const { constants, scrollTo, renderUtils, INTENT_MAP, getNodes, showAll, hideAllButActive, composedPath } = utils;
...
//     // at this point we are dedicating to a vertical scroll
...
const Fullpage = ({ window, document, children, slides, isNext, activeSlide = 0}) => {
  return (
    <div className='Fullpage'>
      {children}
      {slides.map((sl, i) => {
        const isActive = i == activeSlide;
        const nextQualififier = isNext[i];

        const key = `vertical-slide-${i}`;
        const slProps = Object.assign({ window, document, isNext: nextQualififier, key, isActive }, sl.props || {});
        const slChildren = sl.children;

        if (sl.type.name === 'HorizontalSlider') {
          const { name } = sl;
          const hslProps = Object.assign({}, slProps, name);
          return <HorizontalSlider {...hslProps}/>;
        }

        return <Slide {...slProps}>{slChildren}</Slide>
      })}
    </div>
  )
}
...
function determineVerticalRoot() {
  ...
  const browser = utils.detectBrowser(agent);
...
/**
 * Event Wrapper for Fullpage functionality
 * @class FullpageEmitter
 * @extends {React.Component}
 */
class FullpageEmitter extends React.Component {
  constructor(props) {
    super(props);

    if (props.infinite && props.resetSlides) {
      throw new Error(
        'Fullpage Component cannot have both infinite and resetSlides as truthy props'
      );
    }
    ...
    this.scrollSensitivity = props.scrollSensitivity;
    this.touchSensitivity = props.touchSensitivity;
  ...
  componentDidMount() {
    this.window.addEventListener('resize', this.resize.bind(this, this.window));
    const ss = new ScrollSwipe({
      target: this.node,
      scrollSensitivity: this.scrollSensitivity * 3,
      touchSensitivity: this.touchSensitivity * 3,
  ...
  handleScroll(node, winProp, to, newState, compName, cb = noOp) {
    ...
    scrollTo(node, winProp, to, this.scrollSpeed, () => {
      ...
      this.setState(newState, () => {
        cb();
        this.onSlideChange(compName, this.props, this.state, newState[compName] || newState);
        setTimeout(() => {
          ss.flush();
          ss.listen();
        }, 200);
  ...
  render() {
    ...
    const isNext = props.infinite ? {
      [state.activeSlide + 1]: 1,
      [state.activeSlide - 1]: 1
    } : {
      [state.activeSlide + 1]: 1
    };

    const fullProps = Object.assign({ isNext }, props, state);
    return (
...
FullpageEmitter.propTypes = {
  activeSlide: PropTypes.number,
  infinite: PropTypes.bool,
  resetSlides: PropTypes.bool,
  ...
  touchSensitivity: PropTypes.number.isRequired,
  scrollSpeed: PropTypes.number.isRequired
};
FullpageEmitter.defaultProps = {
  activeSlide: 0,
  infininte: false,
  resetSlides: false,
  onSlideChange: () => {},
...
FullpageEmitter.changeHorizontalSlide = () => {};

export default FullpageEmitter;
